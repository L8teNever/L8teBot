...
def manage_twitch(guild_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'set_multi_stream_config':
            ...
            display_mode = request.form.get('display_mode', 'channel')
            auto_assign = request.form.get('auto_assign') == 'on'
            send_offline_message = request.form.get('send_offline_message') == 'on'
            future = asyncio.run_coroutine_threadsafe(cog.web_set_feed_config(guild_id, channel_id, display_mode, auto_assign, send_offline_message), bot.loop)
        ...
        elif action == 'bulk_remove_roles':
            future = asyncio.run_coroutine_threadsafe(cog.web_bulk_remove_streamer_roles(guild_id), bot.loop)
...
@app.route('/guild/<int:guild_id>/wordle', methods=['GET', 'POST'])
@requires_authorization
def manage_wordle(guild_id):
    if not check_guild_permissions(guild_id):
        flash("Du hast keine Berechtigung für diesen Server.", "danger")
        return redirect(url_for('dashboard'))
    ...
    guild = bot.get_guild(guild_id)
    guild_config = bot.data.get_server_config(guild_id)
    cog = bot.get_cog('Wordle')
    is_enabled = 'Wordle' in guild_config.get('enabled_cogs', [])
    ...
    return render_template('wordle_tools.html', 
                           guild=guild, 
                           is_enabled=is_enabled, 
                           settings=wordle_config,
                           admin_guilds=get_admin_guilds())
...
def manage_guard(guild_id):
    ...
    # Onboarding Daten
    onboarding_is_enabled = 'Onboarding' in guild_config.get('enabled_cogs', [])
    onboarding_config = bot.data.get_guild_data(guild_id, "onboarding")
    ...
    return render_template('guard.html', guild=guild, admin_guilds=get_admin_guilds(),
                           ...
                           onboarding_config=onboarding_config, onboarding_is_enabled=onboarding_is_enabled)
...
# --- Onboarding Web-Route ---
@app.route('/guild/<int:guild_id>/onboarding', methods=['GET', 'POST'])
@requires_authorization
def manage_onboarding(guild_id):
    """Verwaltet die Onboarding-Einstellungen für einen Server."""
    if not check_guild_permissions(guild_id):
        flash("Du hast keine Berechtigung für diesen Server.", "danger")
        return redirect(url_for('dashboard'))

    # GET requests redirect to the unified guard page
    if request.method == 'GET':
        return redirect(url_for('manage_guard', guild_id=guild_id), code=301)

    guild = bot.get_guild(guild_id)
    guild_config = bot.data.get_server_config(guild_id)
    is_enabled = 'Onboarding' in guild_config.get('enabled_cogs', [])
    cog = bot.get_cog('Onboarding')

    if request.method == 'POST':
        if not is_enabled or not cog:
            flash("Das Onboarding-Modul ist nicht aktiv.", "danger")
            return redirect(url_for('manage_guard', guild_id=guild_id))
